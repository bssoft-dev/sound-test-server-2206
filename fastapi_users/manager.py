...
class BaseUserManager(Generic[models.UC, models.UD]):
    ...
    async def get_by_username(self, username: str) -> models.UD:
        """
    ...
    async def oauth_callback(
        ...
        try:
            ...
            try:
                # Link account
                user = await self.get_by_username(oauth_account.account_id)
                ...
                user = self.user_db_model(
                    username=oauth_account.account_id,
    ...
    async def forgot_password(
        ...
        msg = f"""
            본 메일은 스마트벨 패스워드 재설정 메일입니다.\n
            아래 링크를 눌러 재설정을 완료해주시기 바랍니다.\n\n
            http://api-2207.bs-soft.co.kr/auth/reset-password?token={token}\n\n
            본 메일은 발신전용입니다. 문의사항은 고객센터로 문의해주시기 바랍니다.\n"""
        send_mail(user.email, "스마트벨 패스워드 재설정 메일", msg)
        print(f"Reset URL has been sent to {user.email}")
